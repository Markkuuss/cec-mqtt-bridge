...
class HdmiCec:
    ...
    def __init__(
        ...
        if cec is None:
            raise RuntimeError(
                "CEC support is not available (python-cec not installed)."
            )
        ...
        # Open connection
        self.cec_client = cec.ICECAdapter.Create(
            self.cec_config
        )  # type: cec.ICECAdapter
        if not port:
    ...
    def _on_command_callback(self, cmd):
        ...
        if not self.refreshing:
            if opcode == cec.CEC_OPCODE_REPORT_POWER_STATUS:
                ...
                self._mqtt_send(
                    f"cec/device/{initiator}/address", f"{physical_address:04x}"
                )
    ...
    def _parse_key_code(self, key: str) -> int:
        ...
        elif re.fullmatch(r"[0-9a-fA-F]{1,2}", key_value) and re.search(
            r"[a-fA-F]", key_value
        ):
            key_code = int(key_value, 16)
    ...
    def scan(self):
        ...
        for device in self.devices:
            ...
            if physical_address != 0xFFFF:
                ...
                self._mqtt_send(
                    f"cec/device/{device}/type",
                    self.cec_client.LogicalAddressToString(device),
                )
                self._mqtt_send(
                    f"cec/device/{device}/address", f"{physical_address:04x}"
                )
                self._mqtt_send(f"cec/device/{device}/active", str(active))
                self._mqtt_send(
                    f"cec/device/{device}/vendor",
                    self.cec_client.VendorIdToString(vendor_id),
                )
                self._mqtt_send(f"cec/device/{device}/osd", osd_name)
                self._mqtt_send(
                    f"cec/device/{device}/cecver",
                    self.cec_client.CecVersionToString(cec_version),
                )
                self._mqtt_send(
                    f"cec/device/{device}/power",
                    self.cec_client.PowerStatusToString(power),
                )
